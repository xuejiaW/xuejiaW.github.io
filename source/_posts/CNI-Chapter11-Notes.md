...
![分层频分复用](CNI-Chapter11-Notes/2019-12-08-12-06-55.png)
...
![时分复用](CNI-Chapter11-Notes/2019-12-08-12-18-29.png)
...
![信道帧](CNI-Chapter11-Notes/2019-12-08-14-42-09.png)
...
为了解决同步TDM这个问题，有个称为`静态时间复用（Statistical TDM）`，或称为异步时间复用（Asynchronous）的技术。这个技术仍然是所有信源轮询的传输数据，但是当信源没有数据时，并非像同步时间同步那样装填空白数据，而是直接跳过。如下图所示：

![静态时间复用](CNI-Chapter11-Notes/2019-12-08-14-54-27.png)
...
![逆复用](CNI-Chapter11-Notes/2019-12-08-15-04-52.png)
...
上述例子中，每个信源的二进制码$c_i$都比较小，但在实际运用中$c_i$可能会很大，这就导致了计算量的上升。因此在数据量不高的网络中CDM并不适用。
