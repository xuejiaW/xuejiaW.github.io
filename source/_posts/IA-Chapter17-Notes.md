...
$$
\sum^{k-1}\_{i=0}\lfloor \frac{n}{2^i} \rfloor \\\\
< n \sum^{\infty}\_{i=0}\frac{1}{2^i} \\\\
=2n
$$
...
$$
\sum^{n}\_{i=1}\hat{c_i}\geq\sum^{n}\_{i=1} c_i
$$

所有操作的credit总和即为$\sum^{n}\_{i=1} \hat{c_i} - \sum^{n}\_{i=1} c_i$。为了保证平摊花费的总和代表上限，这个值必须是非负的。
...
$$
\sum^{n}\_{i=0} \hat{c_i} \\\\
=\sum^{n}\_{i=0}(c_i+\Phi(D_{i})-\Phi(D_{i-1})) \\\\
=\sum^{n}\_{i=0}c_i + \Phi(D_{n}) - \Phi(D_{0})
$$

为了平均花费的总和是真实花费总和的上届，必须保证$\Phi(D_{n})\geq \Phi(D_{0})$。在实际中，并不能知道$n$的值是多少，所以我们要保证$\Phi(D_{i})\geq \Phi(D_{0})$。
...
这里，定义势能为操作后数组中值为1的元素书，表达为$b_i$。假设第$i$个操作，把$t_i$位从1变为0，其真实花费最多为$t_i+1$，即再加上一个退出循环后的从0到1的操作（也可能在退出循环后，已经大于了数组的边界，也就不会有从0到1的操作了）。
...
$$
\sum^{n}\_{i=0}c_i \\\\
=\sum^{n}\_{i=0}\hat{c_i}-\Phi(D_{n})+\Phi(D_{0})
$$
